...
function renderPageHeader(
  ...
  const headerBrand = mode === 'public'
    ? `
      <div style="display:flex;align-items:center;gap:10px;">
        <svg width="22" height="22" viewBox="0 0 24 24" fill="#C41E3A" aria-hidden="true">
          <path d="M12 0L13.5 6.5L17 4L15.5 8.5L22 9L17 12L20 16L14 14L12 24L10 14L4 16L7 12L2 9L8.5 8.5L7 4L10.5 6.5Z"/>
        </svg>
        <div>
          <div class="page-header-consultant">Maple Insight</div>
          <div class="page-header-sub">Settlement Planner</div>
        </div>
      </div>
    `
    : `
...
/** CTA block — consultant follow-up or public next steps */
function renderCTABlock(pkg: MapleReportPackage, mode: PlannerMode = 'consultant'): string {
  if (mode === 'public') {
    const steps = [
      'Compare savings against your estimated landing costs.',
      'Review the breakdown and look for places to reduce expenses.',
      'Work through your first-90-days checklist before you arrive.',
      'Consider an RCIC if you need pathway-specific immigration guidance.',
    ]

    return `
      <div style="padding:18px 22px;border-radius:10px;border:1px solid #1B7A4A20;border-left:5px solid #1B7A4A;background:#FFFFFF;margin-bottom:14px;">
        <div style="font-family:'DM Serif Display',Georgia,serif;font-size:14pt;font-weight:700;color:#1B4F4A;margin-bottom:5px;">What to Do Next</div>
        <div style="font-size:9pt;color:#374151;line-height:1.6;margin-bottom:10px;">Use your results to prepare for your move with more confidence and clarity.</div>
        ${steps.map((step, index) => `
          <div style="display:flex;gap:8px;align-items:flex-start;margin-bottom:${index === steps.length - 1 ? 0 : 7}px;">
            <span style="width:18px;height:18px;min-width:18px;border-radius:999px;background:#E8F5EE;color:#1B7A4A;font-size:8pt;font-weight:700;display:inline-flex;align-items:center;justify-content:center;">${index + 1}</span>
            <span style="font-size:8.5pt;color:#374151;line-height:1.5;">${esc(step)}</span>
          </div>
        `).join('')}
      </div>
    `
  ...
  return `
    <div class="section">
      ...
      <div class="disclaimer">
        ${mode === 'public'
          ? `This report is generated by Maple Insight's Settlement Planner and is intended as a planning estimate only. Actual costs vary by city, household, and personal situation. This document does not constitute immigration, legal, or financial advice. Consider consulting a Registered Canadian Immigration Consultant (RCIC) for program-specific guidance. Engine&nbsp;v${esc(pkg.engineVersion)} · Data&nbsp;v${esc(pkg.dataVersion)}.`
          : `This report is generated by Maple Insight's Settlement Financial Planner and is intended as a planning estimate only. Actual costs may vary significantly based on market conditions. This document does not constitute financial, legal, or immigration advice. All immigration cost figures are sourced from IRCC published fee schedules. Cost-of-living figures are based on Statistics Canada and Maple Insight's proprietary cost models. Engine&nbsp;v${esc(pkg.engineVersion)} · Data&nbsp;v${esc(pkg.dataVersion)}.`}
      </div>
    </div>
  `
}
...
function buildClientPage1(pkg: MapleReportPackage, totalPages: number, mode: PlannerMode): string {
...
function buildClientPage2(
  pkg: MapleReportPackage,
  totalPages: number,
  dataSources: Map<string, DataSource> | undefined,
  mode: PlannerMode,
): string {
  const { consultant, generatedAt } = pkg
  return `
    <div class="page">
      ...
      <div class="page-content">
        ...
        ${renderCTABlock(pkg, mode)}
        ${renderDisclaimers(pkg, dataSources, mode)}
        <div class="page-footer">
...
function buildClientAppendix(
  pkg: MapleReportPackage,
  totalPages: number,
  appendixPageNum: number,
  mode: PlannerMode,
): string {
  ...
  return `
    <div class="page">
      ${renderPageHeader(consultant?.displayName ?? null, consultant?.companyName ?? null, generatedAt, 'Settlement Checklist — Appendix', mode)}
      <div class="page-content">
...
<body>
  ${buildClientPage1(pkg, totalPages, mode)}
  ${buildClientPage2(pkg, totalPages, dataSources, mode)}
  ${buildClientAppendix(pkg, totalPages, 3, mode)}
  ${consultantPages}
</body>
</html>`
}

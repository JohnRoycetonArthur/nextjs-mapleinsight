/**
 * import-articles-batch4.mjs
 *
 * Imports 4 Batch 4 articles from docs/articles-batch4/
 * into Sanity CMS (project: 1rkr5mdi, dataset: production).
 *
 * Usage:
 *   node scripts/import-articles-batch4.mjs            # import
 *   node scripts/import-articles-batch4.mjs --dry-run  # log only, no writes
 *   node scripts/import-articles-batch4.mjs --backup   # backup existing articles first
 */
...
const ARTICLES_DIR = path.join(process.cwd(), 'docs', 'articles-batch4')
...
async function backupArticles() {
  ...
  const file = path.join(BACKUPS_DIR, `articles-backup-batch4-${ts}.json`)
...
async function importArticles() {
  ...
  console.log(`Found ${files.length} article file(s) in docs/articles-batch4/\n`)
  ...
  for (const file of files) {
    ...
    if (DRY_RUN) {
      ...
      console.log(`  relatedCalculator:${data.relatedCalculator || 'null'}`)
